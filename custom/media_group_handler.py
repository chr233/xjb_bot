...
# @LastEditors  : Chr_
# @LastEditTime : 2022-02-13 11:02:37
...
async def _on_media_group_received(media_group_id: str, storage: BaseStorage, user: Users, callback, *args, **kwargs):
...
def media_group_handler(func: Optional[Callable] = None, receive_timeout: int = 2, ttl: int = 5, storage_prefix: str = "mgroup", storage_driver: Optional[BaseStorage] = None):
    def decorator(handler):
        ...
        async def wrapper(message: types.Message, *args, **kwargs):
            ...
            if storage_driver is not None:
                ...
                storage = await _get_storage_from_state(state, prefix=storage_prefix, ttl=ttl)

            if await storage.set_media_group_as_handled(message.media_group_id):
                event_loop.call_later(
                    receive_timeout,
                    asyncio.create_task,
                    _on_media_group_received(message.media_group_id, storage, message.user, handler, *args, **kwargs),
                )
